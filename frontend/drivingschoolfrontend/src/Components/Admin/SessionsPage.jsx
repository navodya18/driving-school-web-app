...
const SessionsPage = () => {
  ...
  const validateSessionDates = () => {
    ...
    // Check if end time is in the past or before start time
    if (isPast(endTime) || endTime <= startTime) {
      setFormError('End time must be in the future and after start time');
      return false;
    }
    ...
    if (startHour < 9 || startHour > 17 || (startHour === 17 && startMinute > 0)) {
      setFormError('Sessions must start between 9:00 AM and 5:00 PM (business hours)');
      return false;
    }
    ...
    const hasOverlap = sessions.some(session => {
      ...
      // 3. New time range completely contains the existing time range
      // 4. Existing time range completely contains the new time range
      return (
    ...
    if (hasOverlap) {
      setFormError('This time slot overlaps with an existing session. Please choose a different time.');
      return false;
    }
  ...
  const handleOpenDialog = (edit = false, sessionId = null) => {
    ...
    if (edit && sessionId) {
      ...
      if (defaultEndTime.getHours() > 17 || (defaultEndTime.getHours() === 17 && defaultEndTime.getMinutes() > 0)) {
  ...
  const handleSaveSession = async () => {
    ...
    try {
      ...
      if (err.response?.data?.message?.includes('time slot') || 
        ...
        setFormError(err.response.data.message || 'This time slot conflicts with an existing session');
      } else {
  ...
  const handleDateClick = (arg) => {
    ...
    const now = new Date();
    const isSameDay = now.getDate() === clickedDate.getDate() && 
  ...
  return (
    <Box>
      <Box display="flex" justifyContent="space-between" alignItems="center" mb={3}>
        ...
        <Box>
          <Typography variant="body2" color={colors.grey[300]} sx={{ mb: 1, textAlign: 'right' }}>
          ...
          <FullCalendar
            ...
            eventDidMount={(info) => {
              ...
              if (info.event.extendedProps.hasOverlap) {
                ...
                tooltip.innerHTML = '<div class="tooltip-content">Warning: This session overlaps with another session!</div>';
